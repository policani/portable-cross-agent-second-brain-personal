...
* ``census`` (default false): when true, every accessible regular file under
  ``vault_root`` (including hidden, cache, dependency, build, and loose
  root-level files) is listed (name/size only, content not read) so the
  console reports the physical workspace total, not only the curated memory.
  Census entries cost no retrieval tokens.
* ``exclude``: folder/file names to skip from content and retrieval scanning,
  merged with the defaults (node_modules, .git, __pycache__, build outputs,
  legacy-*). It does not reduce the physical census.
...
# Only these patterns are universally regenerated by a known runtime. Other
# temporary-looking names remain review signals: an application may still be
# using a ``.tmp``, ``.bak``, or lock file when the inventory runs.
SAFE_DISPOSABLE_PATTERNS = ("__pycache__", "*.pyc", "*.pyo")
DEPENDENCY_DIRS = frozenset(("node_modules", ".venv", "venv", ".cache",
                             ".mypy_cache", ".pytest_cache"))
BUILD_DIRS = frozenset(("build", "dist", ".next"))
TEMPORARY_NAME_PATTERNS = ("*.tmp", "*.temp", "*.swp", "*.swo", "*~",
                           "~$*", "*.bak", "*.orig", "*.rej", "*.lock.tmp",
                           "thumbs.db")
...
def census_walk(base: Path, coverage: dict[str, int] | None = None):
    """Yield every accessible regular file beneath *base* for disk inventory.

    This intentionally does not reuse :func:`walk`: the content index must
    skip dependencies, generated output, hidden folders, and caches, while a
    folder-size report must include them to agree with Explorer. Reparse-point
    directories are not followed, which prevents traversing outside the vault
    or counting a linked directory twice.
    """
    try:
        entries = sorted(base.iterdir())
    except OSError:
    ...
    for entry in entries:
        try:
            if entry.is_symlink() or entry.is_junction():
                ...
                continue
            if entry.is_dir():
                yield from census_walk(entry, coverage)
            elif entry.is_file():
                yield entry
        except OSError:
            if coverage is not None:
...
def ordinary_work_path(rel: str, cfg: dict) -> bool:
    """Whether a file would survive the curated, non-hidden workspace walk."""
    return not any(part.startswith(".") or excluded(part, cfg["_excludes"])
                   for part in rel.split("/"))


def triage_signals(path: Path, rel: str) -> list[str]:
    """Return conservative, portable cleanup-review evidence for one file."""
    parts = [part.lower() for part in rel.split("/")]
    name = path.name.lower()
    signals: list[str] = []
    if ("__pycache__" in parts or any(fnmatch.fnmatch(name, pattern)
                                      for pattern in SAFE_DISPOSABLE_PATTERNS)):
        signals.append("safe_runtime_cache")
    if any(part in DEPENDENCY_DIRS for part in parts):
        signals.append("rebuildable_dependency")
    if any(part in BUILD_DIRS for part in parts):
        signals.append("build_output_review")
    if any(fnmatch.fnmatch(name, pattern) for pattern in TEMPORARY_NAME_PATTERNS):
        signals.append("temporary_name_review")
    ext = path.suffix.lower().lstrip(".")
    if ext in ARCHIVE_EXT or ext in SOFTWARE_EXT:
        signals.append("archive_or_installer_review")
    return signals


def hash_file(path: Path) -> str | None:
    """Hash a collision candidate without loading it whole into memory."""
    try:
...
def add_duplicate_groups(files: list[dict], cfg: dict) -> None:
    """Mark exact, material duplicate groups; never select a deletion target."""
    by_size: dict[int, list[dict]] = {}
    for item in files:
        if item["bytes"] >= DUPLICATE_MIN_BYTES:
            by_size.setdefault(item["bytes"], []).append(item)
    group_number = 0
    for same_size in by_size.values():
        ...
        for item in same_size:
            digest = hash_file(cfg["_root"] / item["path"])
            if digest:
        ...
        for matches in by_hash.values():
            ...
            for item in matches:
                ...
                item["triage"].append("exact_duplicate_review")


def inventory_summary(files: list[dict], coverage: dict[str, int]) -> dict:
    """Compact totals for UI/reporting; candidate bytes are never reclaimable."""
    tiers: dict[str, dict[str, int]] = {}
    signals: dict[str, dict[str, int]] = {}
    candidate_files = [item for item in files if item.get("triage")]
    for item in files:
        bucket = tiers.setdefault(item["visibility_tier"], {"files": 0, "bytes": 0})
        bucket["files"] += 1
        bucket["bytes"] += item["bytes"]
        for signal in item.get("triage", []):
            marker = signals.setdefault(signal, {"files": 0, "bytes": 0})
            marker["files"] += 1
            marker["bytes"] += item["bytes"]
    duplicate_excess = 0
    for group in {item.get("duplicate_group") for item in files} - {None}:
        members = [item for item in files if item.get("duplicate_group") == group]
        duplicate_excess += members[0]["bytes"] * (len(members) - 1)
    return {"files": len(files), "bytes": sum(item["bytes"] for item in files),
            "tiers": tiers, "signals": signals,
            "candidate_files": len(candidate_files),
            "candidate_bytes": sum(item["bytes"] for item in candidate_files),
            "duplicate_excess_bytes": duplicate_excess, "coverage": coverage}
...
def build_index(verbose: bool = True) -> dict:
    ...
    for path in (unique_paths[key] for key in sorted(unique_paths)):
        ...
        files.append({
            ...
            "indexed": True,
            "visibility_tier": "memory",
            "modified": int(path.stat().st_mtime),
            "triage": triage_signals(path, rel),
    ...
    census_n = 0
    coverage = {"unreadable_directories": 0, "unreadable_entries": 0,
                "skipped_links": 0}
    if cfg.get("census"):
        for path in census_walk(cfg["_root"], coverage):
            try:
                rel = rel_to_root(path, cfg)
            except ValueError:
                continue
            ...
            files.append({
                ...
                "bytes": path.stat().st_size,
                "indexed": False,
                "visibility_tier": ("work" if ordinary_work_path(rel, cfg)
                                    else "physical_only"),
                "modified": int(path.stat().st_mtime),
                "triage": triage_signals(path, rel),
            })
            census_n += 1
    for item in files:
        if item["bytes"] >= LARGE_FILE_BYTES:
            item["triage"].append("large_file_review")
    add_duplicate_groups(files, cfg)
    ...
    index = {
        ...
        "census_mode": "physical" if cfg.get("census") else "off",
        "inventory": {
            "version": 1,
            "large_file_bytes": LARGE_FILE_BYTES,
            "duplicate_min_bytes": DUPLICATE_MIN_BYTES,
            "tiers": {
                "memory": "Content-indexed memory and instructions.",
                "work": "Ordinary workspace files outside hidden, dependency, cache, and build paths.",
                "physical_only": "Accessible physical files intentionally outside the work view.",
            },
            "signals": {
                "safe_runtime_cache": {"level": "safe", "label": "Known runtime cache", "detail": "Python bytecode cache; regenerated by Python."},
                "rebuildable_dependency": {"level": "review", "label": "Dependency or environment", "detail": "May be rebuildable when its project manifest is available."},
                "build_output_review": {"level": "review", "label": "Build output", "detail": "Review before removal; some deployments serve generated output."},
                "temporary_name_review": {"level": "review", "label": "Temporary-looking name", "detail": "Name alone is not deletion authorization."},
                "archive_or_installer_review": {"level": "review", "label": "Archive or installer", "detail": "May be intentional source or recovery media."},
                "large_file_review": {"level": "review", "label": "Large file", "detail": "Large files merit review, not automatic removal."},
                "exact_duplicate_review": {"level": "review", "label": "Exact duplicate", "detail": "Same content hash; ownership review still required."},
            },
            "summary": inventory_summary(files, coverage),
        },
    ...
    if verbose:
        ...
        if cfg.get("census"):
            msg += f" + census of {census_n} physical workspace files"
        if agents:
            per_tool = {}
            for a in agents:
